...
import com.paper.reviewer.database.entity.ReviewReportEntity;
import com.paper.reviewer.database.mapper.ReviewReportMapper;
...
public class MyBatisReviewReportRepository implements ReviewReportRepository {
    ...
    @Override public ReviewReport save(ReviewReport report) {
        ...
        e.setRecommendation(report.recommendation()); e.setConfidenceScore(report.confidenceScore());
    ...
    private ReviewReport toDomain(ReviewReportEntity e) {
        return new ReviewReport(e.getId(), e.getReviewId(), e.getReviewerRole(), e.getContentMarkdown(),
                e.getScoresJson(), e.getRecommendation(), e.getConfidenceScore(), e.getStatus(),
